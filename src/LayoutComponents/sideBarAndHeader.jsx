...
const SideBarAndHeader = () => {
  ...
  return (
    ...
    {/* Top HamBurger icon when screen will small */}
      <div className='w-72 '>
        <button className='mt-3.5 ml-2  focus:outline-none focus:bg-gray-600 hover:bg-gray-600 rounded-md lg:hidden'
              onClick={toggleOpen}>
...
{/* ${loginOnOff ? 'translate-x-0':'-translate-x-full'} */}
    ...
    <div className={`overflow-hidden lg:mt-14 md:mt-2.5 mt-2.5 rounded-xl antialiashed  min-h-screen relative lg:flex `} >
      <nav
        className={`absolute flex flex-col inset-0 duration-300 ease-in lg:transform-none lg:opacity-100 transform ${open ? 'translate-x-0' : '-translate-y-full '}  lg:relative z-10 w-72 bg-[#ece7e7] text-black h-screen p-3`}
      >
  ...
  {/* Sidebar top section */}
    ...
    {/* Sidebar top section */}
        ...
        <div className='mt-5 gap-3 border-1 bg-white rounded-2xl shadow-xl p-2'>
              ...
              <NavLink to='/home' className={({isActive}) =>`mt-1 group relative flex items-center mb-2 gap-2 px-4 py-2.5  font-semibold text-bodydark1 transition-color ${isActive ? "bg-[#323131] text-white rounded-r-xl" : "text-bodydark1"}duration-500 ease-in-out hover:bg-[#323131] hover:text-gray-100 rounded-r-xl`}>
    ...
    <div className={`mt-3 gap-3 border-1 duration-0  bg-white rounded-lg  b-0 shadow-xl p-2 text-red-800`}>
        ...
        <NavLink to='/logIn' className={`group relative flex items-center mb-0  gap-2 px-4 py-2.5  font-semibold text-bodydark1 transition-color duration-500 ease-in-out hover:bg-red-700 hover:text-white rounded-r-xl` } onClick={SetLogOnOff}>
